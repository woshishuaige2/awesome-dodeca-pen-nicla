...
def main():
	...
	tip_loc_cent = np.array([0, 135.8, -84.0]).reshape(3,) 
	...
	#capture a video with the Dodec pen pressing a fix point and rotate freely

	cap = cv2.VideoCapture('./src/pentip_calib.MOV')
	ret = 1
	j = 0 
	while(ret):		
		ret,frame = cap.read()
