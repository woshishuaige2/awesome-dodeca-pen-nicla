...
DEFAULT_ACCEL_CALIBRATION_PATH = (
    Path(__file__).resolve().parents[1] / "calibration" / "accel_bias.json"
)
...
def load_accel_calibration(path=None):
    calibration_path = Path(path) if path is not None else DEFAULT_ACCEL_CALIBRATION_PATH
    if not calibration_path.exists():
        return {
    ...
    payload = json.loads(calibration_path.read_text())
    payload["bias"] = np.asarray(payload.get("bias", [0.0, 0.0, 0.0]), dtype=float)
    payload["scale"] = float(payload.get("scale", 1.0))
    payload["path"] = str(calibration_path)
    return payload


def correct_accel(accel, calibration):
    if accel is None:
        return None
    return (np.asarray(accel, dtype=float) - calibration["bias"]) * calibration["scale"]
